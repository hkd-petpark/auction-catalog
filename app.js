...
function ensureHeadersStrictRelaxed(header) {
  const required = ['種類','毛色','性別','生年月日','血統書団体名'];
...
function createCard(item, no) {
  ...
  img.className = 'thumb';
  img.src = item.image || 'images/no-photo.png';
...
// ★画像の元URLを、クリック時に拾えるよう data-src に保存（委譲用）
img.dataset.src = item.image || 'images/no-photo.png';
...
img.addEventListener('click', () => {
  openLightbox(item.image || 'images/no-photo.png');
});
  ...
  h3.textContent = item.species || '(種類未記入)';
...
function renderFromCSV(text) {
  ...
  const idx = {
    ...
    image: header.indexOf('画像URL') // 無い場合は -1
  };
  ...
  const items = data.map((row) => {
    ...
    return {
      ...
      image:   get(idx.image) // 空なら createCard 側で no-photo にフォールバック
    };
  });
