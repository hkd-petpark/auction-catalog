...
function createCard(item, no) {
 ...
 img.addEventListener('click', () => { openLightbox(item.image || 'images/no-photo.png'); });


 // 画像（サムネイル）：仕切書Noに一致するファイルを自動解決
 const img = document.createElement('img');
 img.className = 'thumb';
 img.alt = item.species || 'thumbnail';
 img.style.width = '100%';
 ...
 (async () => {
   const resolved = await resolveImageByNo(item.shikishoNo); // ← ここで探索
